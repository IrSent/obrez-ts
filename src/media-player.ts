...
import { audioBuffersToWav } from "./audio.ts";

const transcribeButton = document.querySelector(
  "#transcribe"
) as HTMLButtonElement;
// const focusClosestButton = document.querySelector(
//   "#focus-closest"
// ) as HTMLButtonElement;
const addDictBtn = document.querySelector("#add-dict-btn") as HTMLButtonElement;
const newSlugInput = document.querySelector("#new-slug") as HTMLInputElement;
const dictTableBody = document.querySelector(
  "#dict-table-body"
...
let currentAudioTrack: InputAudioTrack | null = null;
let currentAudioResourceName = "";
let currentTranscribeResults: Array<[number, number, string]> | null = null;
let loadedDictionaries: Record<string, FastAhoScanner> = {};
let activeDictionaries: Set<string> = new Set();
...
const initMediaPlayer = async (resource: File | string) => {
  try {
    ...
    if (audioTrack) {
      ...
      transcribeButton.style.display = "";
    } else {
...
const updateTranscribeFocus = (playbackTime: number) => {
  if (currentTranscribeResults) {
    const currentIndex = currentTranscribeResults.findIndex(
    ...
    if (currentIndex !== lastActiveIndex) {
      ...
      // Снимаем старую подсветку "активного" ряда
      resultsBody
        .querySelectorAll("tr")
        .forEach((r) => r.classList.remove("active-playing"));

      if (currentIndex !== -1) {
        const activeRow = resultsBody.querySelector(
          `tr[data-index="${currentIndex}"]`
        );
        if (activeRow) {
          activeRow.classList.add("active-playing");
          // Опционально: автоскролл к текущей фразе
...
window.addEventListener("keydown", (e) => {
  ...
  if (isTyping) {
    return; // Если печатаем — ничего не делаем и выходим
  }
...
async function loadDictionary(slug: string): Promise<void> {
  ...
  if (!response.ok) {
    console.warn(`Dict ${slug} not found`);
    return;
  }

  const buffer = await response.arrayBuffer();
  loadedDictionaries[slug] = new FastAhoScanner(buffer);
  activeDictionaries.add(slug);
  renderDictTable();
  renderTranscribeResults();
}

(window as any).removeDictionary = (slug: string) => {
  delete loadedDictionaries[slug];
  activeDictionaries.delete(slug);
  renderDictTable();
  renderTranscribeResults();
};

(window as any).toggleDictionary = (slug: string) => {
  if (activeDictionaries.has(slug)) {
    activeDictionaries.delete(slug);
  } else {
    activeDictionaries.add(slug);
  }
...
function renderDictTable() {
  ...
  Object.keys(loadedDictionaries).forEach((slug: string) => {
    const row = document.createElement("tr");
    row.className =
      "border-b border-zinc-700 hover:bg-zinc-800/50 transition-colors";

    row.innerHTML = `
            <td class="px-4 py-3 text-sm font-medium text-zinc-300">
                <span class="flex items-center gap-2">
                    <span class="size-2 rounded-full ${
                      ...
                      onclick="toggleDictionary('${slug}')"></span>
                    ${slug}
                </span>
            </td>
            <td class="px-4 py-3 text-right">
                <button
                    onclick="removeDictionary('${slug}')"
...
const renderTranscribeResults = () => {
  resultsBody.innerHTML = ""; // Очистка
  resultsContainer.style.display = "";
  // focusClosestButton.style.display = "";
  currentTranscribeResults?.forEach(([start, end, text], index) => {
    let triggered: Array<string> = new Array();
    activeDictionaries?.forEach((activeDict) => {
      const matches = loadedDictionaries[activeDict].findMatches(
        text.toLowerCase()
      );
      if (matches.length > 0) triggered.push(activeDict);
    });
    if (activeDictionaries.size === 0 || triggered.length > 0) {
      const row = document.createElement("tr");
      row.setAttribute("data-index", String(index));
      row.innerHTML = `
              <td class="px-5 py-2 text-sm">${start / 1000}s</td>
              <td class="px-5 py-2 text-sm">${end / 1000}s</td>
              <td class="px-5 py-2 text-sm">
                  <span class="px-2 py-1 rounded bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs">
                      ${text}
                  </span>
              </td>
              <td class="px-5 py-2 text-sm">
                ${
                  triggered.length > 0
                    ? triggered
                        .map(
                          (slug) =>
                            `<span class="rounded text-xs bg-white-100 text-white-700">` +
                            slug +
                            `</span>`
                        )
                        .reduce((acc, s) => acc + s, "")
              ...
              <td class="px-5 py-2 text-sm">
                  <button class="text-purple-500 hover:underline" onclick="window.jumpToTime(${
                    start / 1000
                  })">
                      To time
                  </button>
...
(window as any).jumpToTime = async (time: number) => {
...
addDictBtn.addEventListener("click", async () => {
  ...
  if (slug) {
    ...
    addDictBtn.textContent = "Loading";
    await loadDictionary(slug);
    renderDictTable();
    newSlugInput.value = "";
    addDictBtn.disabled = false;
    addDictBtn.textContent = "Add";
  }
});

transcribeButton.addEventListener("click", async () => {
  if (!currentAudioTrack || !audioSink) return;
  ...
  try {
    ...
    const audioBlob = audioBuffersToWav(chunks, currentAudioTrack.sampleRate);
    const formData = new FormData();
    formData.append("file", audioBlob, `${currentAudioResourceName}.wav`);
    const response = await fetch("http://localhost:8686/transcribe", {
      method: "POST",
      body: formData,
    });
    const { task_id } = await response.json();
    transcribeButton.textContent = "Task in progress";
    ...
    socket.onmessage = (event) => {
      ...
      if (msg.status === "PROCESSING") {
        ...
        transcribeButton.textContent = "Done.";
        socket.close();
        currentTranscribeResults = msg.results;
        renderTranscribeResults();
      }
    };
  } catch (err) {
    console.error("Processing failed:", err);
    transcribeButton.textContent = "Error. Check Console";
  }
});

// focusClosestButton.addEventListener("click", () => {
//   if (!currentTranscribeResults || currentTranscribeResults.length < 2) return;

//   let minDiff = Infinity;
//   let targetIndex = -1;
//   for (let i = 1; i < currentTranscribeResults.length; i++) {
//     const diff =
//       currentTranscribeResults[i][0] - currentTranscribeResults[i - 1][1];
//     if (diff < minDiff) {
//       minDiff = diff;
//       targetIndex = i;
//     }
...
//   if (targetIndex !== -1) {
//     const row1 = resultsBody.querySelector(
//       `tr[data-index="${targetIndex - 1}"]`
//     );
//     const row2 = resultsBody.querySelector(`tr[data-index="${targetIndex}"]`);

//     row1.classList.add("row-highlight");
//     row2.classList.add("row-highlight");
...
//       row1.classList.add("row-highlight");
//       row2.classList.add("row-highlight");
//       row1.scrollIntoView({ behavior: "smooth", block: "center" });
//     }
//   }
// });
...
document.addEventListener('DOMContentLoaded', async (event) => {
  await loadDictionary('ru-profanity');
  await loadDictionary('ru-stopwords');
});
