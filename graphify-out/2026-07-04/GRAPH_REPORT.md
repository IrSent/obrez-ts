# Graph Report - obrez-ts  (2026-07-03)

## Corpus Check
- 61 files · ~103,671 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 334 nodes · 475 edges · 53 communities (42 shown, 11 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 11 edges (avg confidence: 0.89)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d4e67f79`
...
Cohesion: 0.10
Nodes (29): backendPath, MediaPlayerContext, MediaPlayerProvider(), useMediaPlayerContext(), DEFAULT_DICTIONARIES, DictionaryManager, DictionaryManagerInner(), ExportButton (+21 more)
...
Cohesion: 0.29
Nodes (6): BACKEND_URL, backendWsPath, __dirname, handleTranscriptionRequest(), Bun TLS Server (port 3000), server
...
Cohesion: 0.31
Nodes (7): Window, audioBuffersToWav(), WavProgress, writeString(), yieldToEventLoop(), backendPath(), backendWsPath()
...
Cohesion: 0.09
Nodes (16): CODEC_LABELS, ExportFormat, ExportModal, ExportModalProps, computeSegmentBoundaries(), ensureBleepDecoded(), exportCensoredVideo(), getSoundEffects() (+8 more)

## Knowledge Gaps
- **112 isolated node(s):** `Window`, `Root Cause`, `Fix 1: Add render-quantum margin to `actualEndCorrection` (main fix)`, `Fix 2: Wait for all `onended` before releasing the lock`, `Fix 3: Increase `stopAudio` node-stop wait from 10ms to 20ms` (+107 more)
...
- **What connects `Window`, `Root Cause`, `Fix 1: Add render-quantum margin to `actualEndCorrection` (main fix)` to the rest of the system?**
